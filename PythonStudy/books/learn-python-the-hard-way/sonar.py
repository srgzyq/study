...
def getNewBoard():
    # Create a new 60*15 board data structure.
    board = []
...
def makeMove(board, chests, x, y):
    ...
    # from the chests list as the are found. Return False if this is an invalid move.
...
def showInstructions():
    print('''Instructions:
    You are the captain of the Simon, a treasure-hunting ship. Your current mission is to find the three sunken treasure chests that are lurking in the part of the ocean you are in and collect them. 
    ...
    For example, the d below marks where the device was dropped, and the 2's represent distance of 2 away from the device. The 4's represent distance of 4 away from the device.
    ...
    The treasure chests don't move around. Sonar devices can detect treasure chests up to a distance of 9.If all chests are out of range, the point will be marked with O

    If a device is directly dropped on a treasure chest, you have discovered the location of the chest, and it will be collected.The sonar device will remain there.
...
def testGetRandomChests():
    chests = getRandomChests(3)
    print(chests)
